...
import cn.hzxinghao.mall.config.KafkaProperties;
...
class XhmallApplicationKafkaResourceIT {
    ...
    void setup() {
        ...
        XhmallApplicationKafkaResource kafkaResource = new XhmallApplicationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/xhmall-application-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/xhmall-application-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
